...
export default function ViewComplaints() {
  ...
  const getinfo = async()=>{
    const response = await axios.get('http://localhost:5000/api/auth/OnlineBookingTime/' + name)
  ...
  const Delete =async (id)=>{
    ...
    if(answer===true){
      await axios.delete(`http://localhost:5000/api/auth/onlinebooking/${id}`)
      .then(()=>{
            getinfo();
      }) }
    ...
    const EnhancedTableToolbar = () => {
      ...
      const [OnlineTime, setTime] = useState('');
      const {name} = useParams();
        ...
        <Toolbar
          ...
          />
            ...
            <DialogActions>
              ...
              <Button onClick={OnlineBoking} color="primary">
                Submit
              </Button>
            </DialogActions>
          </Dialog>
