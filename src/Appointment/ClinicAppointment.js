...
const useStyles = makeStyles((theme) => ({
  ...
  function ClinicAppointment() {
    ...
    const getinfo = async()=>{
      ...
      const response =await axios.get('http://localhost:5000/api/auth/BookingTime/' + doc);
    ...
    const appoint_list = async()=>{
      ...
      // Make a request for a user with a given ID
      const response = await axios.get('http://localhost:5000/api/auth/Appointments/' + doc + '/' + de);
      if (response.status === 200) {
    ...
    const book_appointment = async()=>{
      try{
      const response = await axios.post('http://localhost:5000/api/auth/Appointments', {
      ...
      alert("Appointment Booked, Confirmation message will be sent to you.."));
      ...
      alert("You cannot Book more than one Appointments at same Date/Time")
       console.log("Error");
      }
    }catch(error) {
     ...
     <br/>
      ...
      <div className='col-md-5 mx-auto align-self-center'>
        ...
        />
            ...
            {final.map(function(element, i) {
                  return element.Time === selected_time ?
                  <Chip key={i} className={classes.dates} label={element.Time} color="secondary" component="a" onClick={()=>{setselected_time(element.Time)}} clickable />
                    :
                  <Chip key={i} className={classes.dates} label={element.Time} color="primary" component="a" onClick={()=>{setselected_time(element.Time)}} clickable />
                })} 
      ...
      <Paper className={classes.paper} elevation={14}>
            <Typography component="h6" variant="h6" align="center" style={{fontFamily:'Segoe UI',}}>
              Enter Details To Book Clinic Appointment
            </Typography>
            
            <Divider variant="middle" style={{marginLeft:'30%',marginRight:'30%',backgroundColor:'#3f51b5'}} /> 
      <br/>
            <form id="Appoint" className={classes.root} autoComplete="off"  onSubmit={book_appointment}>
              <TextField  required 
        ...
        onChange={(event) => {setphone(event.target.value)}} className={classes.fileds} id="outlined-basic" label="Phone Number" variant="outlined" />
              <TextField    required   type="email"
        onChange={(event) => {setemail(event.target.value)}} className={classes.fileds} id="outlined-basic" label="Email" variant="outlined" />
              <br/>
